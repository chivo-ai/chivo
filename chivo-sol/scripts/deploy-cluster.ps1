...
$clusterKey = if ($Cluster -eq "testnet") { "SOLANA_TESTNET_RPC_URL" } else { "SOLANA_DEVNET_RPC_URL" }
$clusterUrl = [Environment]::GetEnvironmentVariable($clusterKey, "Process")
if (-not $clusterUrl) {
  $clusterUrl = if ($Cluster -eq "testnet") { "https://api.testnet.solana.com" } else { "https://api.devnet.solana.com" }
}

if (-not $env:ANCHOR_WALLET) {
  throw "Set ANCHOR_WALLET to your funded Solana $Cluster authority keypair path."
}

if (-not $env:CHIVO_SOL_PROGRAM_KEYPAIR) {
  throw "Set CHIVO_SOL_PROGRAM_KEYPAIR to your Solana program keypair path."
}
...
Write-Host "Deploying chivo_payments to Solana $Cluster"
Write-Host "Program ID: $programId"
Write-Host "Authority wallet: $env:ANCHOR_WALLET"
Write-Host "RPC: $clusterUrl"
...
anchor build
anchor deploy --provider.cluster $clusterUrl --provider.wallet $env:ANCHOR_WALLET --program-keypair $env:CHIVO_SOL_PROGRAM_KEYPAIR
