...
type ProcessCrewVoiceNoteRequest = {
  resourceId: string;
  language?: string;
};
...
serve(async (request) => {
  ...
  try {
    ...
    const body = (await request.json()) as ProcessCrewVoiceNoteRequest;
    ...
    const { data: resource, error: resourceError } = await supabase
      .from('crew_resources')
      .select('id, crew_id, created_by, title, resource_type, content, created_at')
    ...
    if (resource.resource_type !== 'voice_note') {
      return json({ error: 'Only crew voice notes can be transcribed' }, 400);
    }

    const { data: membership, error: membershipError } = await supabase
      .from('crew_memberships')
      .select('id')
      .eq('crew_id', resource.crew_id)
    ...
    const audio = await fetchCrewAudio(supabase, storagePath, mimeType);
    ...
    await supabase.from('crew_messages').insert({
      crew_id: resource.crew_id,
      sender_profile_id: user.id,
      body: `Chivo AI transcribed a voice note: ${updatedResource.title}`,
...
async function fetchCrewAudio(
  supabase: any,
  storagePath: string,
  mimeType: string
): Promise<AudioInput> {
  const { data, error } = await supabase.storage.from('chivo-crew-audio').download(storagePath);
...
async function generateVoiceNote(input: {
  ...
  const prompt = [
    ...
    'Listen to this crew voice note and create a clean shared study transcript.',
    'Return JSON only with: title, transcript, summary, key_points.',
    'Keep the transcript faithful and useful for the whole crew.',
...
async function uploadAudioToGemini(geminiKey: string, audio: AudioInput, displayName: string) {
  const startResponse = await fetch(`https://generativelanguage.googleapis.com/upload/v1beta/files?key=${geminiKey}`, {
    ...
    body: JSON.stringify({
      file: {
        display_name: displayName.slice(0, 80) || 'crew-voice-note',
      },
    }),
  });
